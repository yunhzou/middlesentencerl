...
import argparse, json, logging, random, torch, numpy as np, mmap
...
def _memmap_idx(path: Path) -> np.memmap:
    # sentence-start indices (uint32) over ANSWER text, last entry == len(answer)
    return np.memmap(path, dtype=np.uint32, mode="r")

# --------- dataset ----------
class ContiguousMiddleSpanDataset(torch.utils.data.IterableDataset):
    """
    Target is a contiguous block of exactly `mid_len` SENTENCES from ANSWER
    (indexed by a sentence-start .idx produced by Punkt + '\]' marker).
    Context is "Question:\\n{question}\\n" + ANSWER at load time.

    samples_per_doc: int or "full"
    ...
    def __init__(
        ...
        index_dir: Union[str, Path] = "YourDataset_With_QA_Index",
        seed: int = 0,
        ...
        self.meta: List[Dict] = []
        with mpath.open("r", encoding="utf-8") as fp:
            for line in fp:
                m = json.loads(line)
                n_sent = int(m.get("n_sent", m.get("n_lines")))  # allow fallback
                self.meta.append({
                    "id":       m.get("id", Path(m["txt"]).stem),
                    "txt":      m["txt"],        # ANSWER text file
                    "idx":      m["idx"],        # sentence-start memmap for ANSWER
                    "n_sent":   n_sent,          # number of sentence boundaries + 1
    ...
    def _samples_from_doc(self, meta: Dict, rng: random.Random):
        ...
        prefix = f"Question:\n{question}\n"
        context = prefix + answer
        offset = len(prefix)

        n_sent = meta["n_sent"]
        n_valid = max(0, n_sent - self.mid_len)
        if n_valid == 0:
            return
        ...
        L = self.mid_len
        for s in starts:
            c0_ans, c1_ans = self._span_char_bounds(idx, int(s), L)
            c0, c1 = c0_ans + offset, c1_ans + offset
...
# ---------------- demo ----------------
def demo():
    n_samples_for_demo = 5
    return_mode = "triples"
    ds = ContiguousMiddleSpanDataset(
        mid_len=2,
        samples_per_doc=2,   # or "full"
        index_dir="/home/yunhengzou/middlesentencerldata/MegaScience_Index",
        seed=123,
